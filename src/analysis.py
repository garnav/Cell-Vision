...
class Analysis(object):
    ...
    def __init__(self, foldpath, filename):
        index = str.find(filename, ".")
        self.fname = filename[:index]
        self.loadedImage = Image.open(os.path.join(foldpath, filename)).convert('L')
        
    def labelize(self):
        edgeImage = self.imageEdges()
        segmentsImage = self.imageGeneticSegments(edgeImage)
        label = self.imageLabels(segmentsImage)
        self.createRegions(label[0])
        return label
    ...
    def createRegions(self, labelImage):
        self.regions = regionprops(labelImage)

    def drawLabels(self, labelImage, image_label_overlay, location):
        ...
        for region in self.regions:
            ...
            if diameter >= self.MIN_DIAMETER and diameter <= self.MAX_DIAMETER and region.extent >= self.MIN_FILLED_RATIO: 
                ...
                ax.add_patch(rect)
                pylab.text(minc, minr, str(counter), color = 'white')
                counter = counter + 1
        pylab.savefig(os.path.join(location, self.fname + '.png'))
        pylab.show()
    
    def writeProperties(self, labelImage, image_label_overlay, location):
        with open(os.path.join(location, self.fname + '.csv'), 'w') as csvfile:
